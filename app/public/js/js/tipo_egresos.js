...
formAgregarUsuario.addEventListener("submit", async function (event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      Toast.fire({
        icon: "success",
        title: "Se creo el tipo de egreso correctamente",
      });
      getAll();
    } else {
      ...
      const Toast = Swal.mixin({
        ...
        Toast.fire({
            icon: "error",
            title: "Error al crear nueva egreso"
        });
      getAll();
    }
  } catch (error) {
...
//*************renderizado de tabla tipo de egresos y mostrar tabla tipo de egresos*******************/
const ingresos = document;

const paginaTipoingresos = ingresos.querySelector("#tipo_egreso");
...
const Ingresos = ({
  ...
  return `
        <tr id="ingresos-row-${id_tipo_egresos}"> <!-- Agregar un ID único para la fila -->
            <td>${id_tipo_egresos}</td>
            <td>${nombre}</td>
            <td>${formattedDate}</td>
            <td>
            ...
            <td>
              <div class="btn-group">
                  ...
                  <ul class="dropdown-menu ">
                      <li><a id="actualizar" class="dropdown-item" onclick="toggleEditMode(${id_tipo_egresos})" href="#" class="dropdown-item">Actualizar</a></li>
                      <li><a onclick="deleteUser(${id_tipo_egresos})" class="dropdown-item" href="#">Eliminar</a></li>
                      <li><a onclick="changeState(${id_tipo_egresos}, ${estado})" class="dropdown-item" href="#" id="change-state-${id_tipo_egresos}">${estado ? "Inhabilitar" : "Habilitar"}</a></li>
                  </ul>
              </div>
            </td>
        </tr>
...
const render = (data) => {
  const sortetipoingresos = data.sort((a, b) => {
    ...
    return a.id_tipo_egresos - b.id_tipo_egresos;
  });
  ...
  if (!$.fn.DataTable.isDataTable("#myTable")) {
    ...
    paginaTipoingresos.innerHTML =
      '<tr><td colspan="6">NO SE ENCONTRARON TIPO DE EGRESOS.</td></tr>';
  }
};
...
const toggleEditMode = (id_tipo_egresos) => {
  const row = document.getElementById(`ingresos-row-${id_tipo_egresos}`);
  ...
  if (!isEditMode) {
    // Modo de edición
    editIngresos(id_tipo_egresos);
    editButton.innerHTML = "Guardar";
    editButton.setAttribute(
      "onclick",
      `toggleEditMode(${id_tipo_egresos}, ${JSON.stringify(valoresOriginales)})`
    );
    isEditMode = true;
  } else {
    // Modo de guardar cambios
    saveChanges(id_tipo_egresos, valoresOriginales);
    editButton.innerHTML = "Actualizar";
    editButton.setAttribute("onclick", `toggleEditMode(${id_tipo_egresos})`);
    isEditMode = false;
  }
};
//*****************************editar tipo de ingresos y guardar********************************/
const editIngresos = (id_tipo_egresos) => {
  const row = document.getElementById(`ingresos-row-${id_tipo_egresos}`);
  const cells = row.getElementsByTagName("td");
  ...
  editButton.setAttribute(
    "onclick",
    `toggleEditMode(${id_tipo_egresos}, ${JSON.stringify(valoresOriginales)}, this)`
  );
};

// Función para guardar los cambios realizados en la fila
const saveChanges = async (id_tipo_egresos, valoresOriginales) => {
  const row = document.getElementById(`ingresos-row-${id_tipo_egresos}`);
  const cells = row.getElementsByTagName("td");
  ...
  try {
    ...
    if (isConfirmed) {
      ...
      const token = obtenerTokenre();
      const response = await fetch(
        `${baseURL}/tipo_egreso/${id_tipo_egresos}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.status !== 200) {
        ...
        Toast.fire({
          icon: "error",
          title: "Error al actualizar el tipo de egreso",
        });
      }
      // Destruir DataTable antes de eliminar la fila
      if ($.fn.DataTable.isDataTable("#myTable")) {
      ...
      Toast.fire({
        icon: "success",
        title: "Se actualizo el tipo de egreso correctamente",
      });
      getAll();
    } else {
    ...
    Toast.fire({
      icon: "error",
      title: "Ocurrio un error al actualizar el tipo de egreso",
    });
...
//*****************************editar tipo de egresos y guardar********************************/
...
const changeState = async (userId, currentState) => {
  try {
    ...
    const { isConfirmed } = await Swal.fire({
      title: "¿Estás seguro?",
      text: `¿Deseas ${buttonText.toLowerCase()} el tipo de egreso ${userId}?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    if (isConfirmed) {
      ...
      const token = obtenerTokenre();
      const response = await fetch(
        `${baseURL}/tipo_egreso/${userId}/state`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        Toast.fire({
          icon: "error",
          title: "Error al cambiar el estado de el tipo de egreso",
        });
        getAll();
      }
    }
...
const deleteUser = async (userId) => {
  try {
    ...
    const { isConfirmed } = await Swal.fire({
      title: "¿Estás seguro?",
      text: `¿Deseas eliminar el tipo de egreso ${userId}?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    if (isConfirmed) {
      ...
      const token = obtenerTokenre();
      const response = await fetch(
        `${baseURL}/tipo_egreso_delete/${userId}`,
        {
          method: "DELETE",
          headers:{
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        Toast.fire({
          icon: "error",
          title: "Error al eliminar tipo de egreso",
        });
