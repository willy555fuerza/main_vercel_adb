...
formAgregarUsuario.addEventListener("submit", async function (event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      Toast.fire({
        icon: "success",
        title: "Se creo la ministerio correctamente",
      });
      getAll();
    } else {
...
let tablaMinisterio; // Variable de control para DataTables
...
const render = (data) => {
  ...
  if (!$.fn.DataTable.isDataTable("#myTable")) {
    ...
    paginaMinisterios.innerHTML =
      '<tr><td colspan="6">NO SE ENCONTRARON MINISTERIOS.</td></tr>';
  }
};
...
const toggleEditMode = (id_ministerio) => {
  ...
  if (!isEditMode) {
    ...
    editButton.setAttribute(
      "onclick",
      `toggleEditMode(${id_ministerio}, ${JSON.stringify(valoresOriginales)})`
    );
    isEditMode = true;
  } else {
...
const editMinisterio = (id_ministerio) => {
  ...
  editButton.setAttribute(
    "onclick",
    `toggleEditMode(${id_ministerio}, ${JSON.stringify(valoresOriginales)}, this)`
  );
};
...
const saveChanges = async (id_ministerio, valoresOriginales) => {
  ...
  try {
    ...
    if (isConfirmed) {
      ...
      const token = obtenerTokenre();
      const response = await fetch(
        `${baseURL}/ministerio/${id_ministerio}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.status !== 200) {
        ...
        Toast.fire({
          icon: "error",
          title: "Error al actualizar la ministerio",
        });
      }
      // Destruir DataTable antes de eliminar la fila
      if ($.fn.DataTable.isDataTable("#myTable")) {
    ...
    Toast.fire({
      icon: "error",
      title: "Ocurrio un error al actualizar la ministerio",
    });
...
//*******************************inavilitar, eliminar*********************************/
// CAmbiar state de la ministerio (deshabilitacion logica)
const changeState = async (userId, currentState) => {
  try {
    ...
    if (isConfirmed) {
      ...
      const token = obtenerTokenre();
      const response = await fetch(
        `${baseURL}/ministerio/${userId}/state`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        Toast.fire({
          icon: "error",
          title: "Error al cambiar el estado de el ministerio",
        });
        getAll();
      }
    }
...
const deleteUser = async (userId) => {
  try {
    ...
    if (isConfirmed) {
      ...
      const token = obtenerTokenre();
      const response = await fetch(
        `${baseURL}/ministerio_delete/${userId}`,
        {
          method: "DELETE",
          headers:{
            Authorization: `Bearer ${token}`,
