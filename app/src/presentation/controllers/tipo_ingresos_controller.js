...
const Usersmodel = require("../models/tipo_ingresos_model"); // Importa el modelo ProductosModel
...
class Users {
  ...
  static async createUser(req, res) {
    try {
      ...
      if (result) {
        ...
        res.status(200).json({ message: "Tipos de ingresos creado correctamente" });
      } else {
        // Error al crear el tipo de ingresos en el modelo
        res.status(500).json({ error: "Error al crear el tipo de ingresos" });
      }
    } catch (error) {
      console.error("Error al crear el tipo de ingresos:", error);
      res.status(500).json({ error: "Error al crear el tipo de ingresos" });
    }
  }
  ...
  static async deleteUser(req, res) {
    try {
      ...
      const responseObj = { message: "Tipo de ingresos eliminada correctamente" };
