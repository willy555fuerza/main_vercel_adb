...
class Usersmodel {
  ...
  static async getAll() {
    try {
      ...
      const result = await pool.query("SELECT * FROM tipo_egreso");
      ...
      if (result.rows.length === 0) {
        return {
          data: null,
          error: true,
          message: "No hay tipos egresos registrados",
        };
      }
  ...
  static async createUser(nombre) {
    ...
    try {
      ...
      console.log("Tipo de egresos creado correctamente");
      return true;
    } catch (error) {
  ...
  // Metodo para actualizar la tipo de egresos
  static async updateUser(id_tipo_egresos, nombre) {
    ...
    try {
      ...
      // Ejecutar la consulta con parámetros
      await pool.query(query, [nombre, id_tipo_egresos]);

      console.log("Tipo de egresos actualizado correctamente");
      return true;
    } catch (error) {
  ...
  static async changeState(userId, state) {
    try {
      ...
      await pool.query(
        `UPDATE tipo_egreso SET estado = ${state} WHERE id_tipo_egresos = ${userId}`
      );
      await disconnectFromPostgres(pool);
      return true;
    } catch (error) {
  ...
  static async deleteUser(userId) {
    try {
      ...
      await pool.query(`DELETE FROM tipo_egreso WHERE id_tipo_egresos = ${userId}`);

      await disconnectFromPostgres(pool);
      return true;
    } catch (error) {
      console.error("Error al eliminar el tipo de egreso:", error);
      return false;
    }
  }
}
